...
# --- Calcula a média por gene e por grupo
...
# Filtrar apenas Psoriatic Arthritis (lesion e non-lesion)
...
# Filtre o seu objeto 'sample_info' original para manter apenas as amostras que estão na sua matriz final.
...
if (is.null(enrich_result) || nrow(as.data.frame(enrich_result)) == 0) {
  ...
  plot_data <- as.data.frame(enrich_result) %>%
    arrange(p.adjust) %>%
    head(10) %>%
    # 1. QUEBRA DE LINHA AUTOMÁTICA: para nomes longos
    mutate(Description = str_wrap(Description, width = 50)) %>%
    # 2. ORDENAÇÃO SIMPLIFICADA: reordena os termos pelo p-valor (menor no topo)
  ...
  ggplot(plot_data, aes(x = Count, y = Description)) +
    # Usamos geom_segment para criar o efeito "lollipop" (opcional, mas elegante)
    geom_segment(aes(x = 0, xend = Count, y = Description, yend = Description),
                 color = "gray", linewidth = 0.5) +
    # geom_point mapeia tamanho para a contagem e cor para o p-valor ajustado
    geom_point(aes(size = Count, color = p.adjust), alpha = 0.9) +
    
    # 3. REFINAMENTO ESTÉTICO: Melhor paleta de cores (Viridis é amigável para daltônicos)
    scale_color_viridis_c(
      name = "FDR",
      trans = "log10", # Transformação logarítmica para melhor distribuição das cores
      guide = guide_colorbar(reverse = TRUE, order = 1) # Inverte a legenda de cores
    ) +
    
    # Garante que a área do ponto seja proporcional à contagem
    scale_size_area(name = "Nº de Genes", max_size = 12) +
    
    # Títulos e rótulos mais claros
    labs(
      title = "GO Term Enrichment Analysis",
      subtitle = "The 10 Most Significant Biological Processes",
      x = "Genes Counting",
      y = NULL # O eixo Y não precisa de título, os próprios termos já o descrevem
    ) +
...
top_tf_summary <- head(tf_summary, 200) # Ajuste o número conforme necessário
...
write.csv(motif2, "tabela_TF_nonlesion.PsA_vs_lesion.PsA_GSE205748.csv", row.names = FALSE)
...
write.csv(motif2, "tabela_TF_lesion.PsA_vs_control_GSE205748.csv", row.names = FALSE)
