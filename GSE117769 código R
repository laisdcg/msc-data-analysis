...
BiocManager::install(c("DOSE", "GOSemSim", "data.table", "RSQLite", "sf"))

BiocManager::version()
"DOSE" %in% rownames(installed.packages())
...
ggplot(top30_fc, aes(x = reorder(Gene, log2FoldChange), y = log2FoldChange, fill = Regulation)) +
  ...
  theme_bw() +
  labs(
    title = "The most prominent DEGs between PsA and Healthy (GSE117769)",
    x = "Gene",
    y = "log2 Fold Change",
    fill = "Regulation"
  ) + 
...
ggplot(dados_para_plotar, aes(x = mean_expression, y = reorder(Gene, -mean_expression))) +
  ...
  labs(
    ...
    subtitle = "For PsA vs Healthy (GSE186063)",
    x = "Average Expression in the Group 'PsA'", # Rótulo do eixo mais claro
    y = "Gene"
  ) +
  theme_minimal(base_size = 14) +
...
write.csv(motif2, "tabela_TF_nonlesion_vs_lesion_PsA_GSE186063.csv", row.names = FALSE)
...
ggplot(dados_para_plotar, aes(x = mean_expression, y = reorder(Gene, -mean_expression))) +
  ...
  labs(
    ...
    subtitle = "For AS vs Healthy (GSE186063)",
    x = "Average Expression in the Group 'AS'", # Rótulo do eixo mais claro
    y = "Gene"
  ) +
  theme_minimal(base_size = 14) +
...
write.csv(motif2, "tabela_TF_normal_skin_AS_vs_lesion_PsA_GSE186063.csv", row.names = FALSE)
...
ggplot(dados_para_plotar, aes(x = mean_expression, y = reorder(Gene, -mean_expression))) +
  ...
  labs(
    title = "Average Expression of the Main Downregulated Genes in Ankylosing spondylitis", # Título mais claro
    subtitle = "For AS vs PsA (GSE186063)",
    x = "Average Expression in the Group 'AS'", # Rótulo do eixo mais claro
    y = "Gene"
  ) +
  theme_minimal(base_size = 14) +
...
ha_col <- HeatmapAnnotation(
  Group = annotation_data$Group,
  col = list(Group = c("AS" = "#1f77b4", "PsA" = "red"))
)
